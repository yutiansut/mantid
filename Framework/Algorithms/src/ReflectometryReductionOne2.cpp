...
namespace {
unsigned int numberOfMonitors(const MatrixWorkspace_sptr &detectorWS) {
  auto numOfMonitors = 0u;
  const auto &detectorInfo = detectorWS->detectorInfo();
  for (auto i = 0u; i < detectorInfo.size(); ++i) {
...
ReflectometryReductionOne2::sumInQ(MatrixWorkspace_sptr detectorWS) {
  // If only one detector is present in the workspace then return the input
  // without any changes.
  if (detectorWS->getNumberHistograms() - numberOfMonitors(detectorWS) <= 1) {
    g_log.information("SumInQ called but with only one or less detector it "
                      "does nothing as there is nothing to sum");
    return detectorWS;
  }
  ...
  try {
    ...
    sumInQAlg->setProperty("FlatSample", getPropertyValue("ReductionType") ==
                                             "DivergentBeam");
    sumInQAlg->execute();
  } catch (...) {
    g_log.warning("Something went wrong when running SumInQ, no calculation "
                  "was performed.");
    return detectorWS;
  }

  return sumInQAlg->getProperty("OutputWorkspace");
}
