...
template <size_t ND>
std::pair<bool, bool> isMDEventWSTypeND(API::IMDEventWorkspace_sptr ws) {
  if(boost::dynamic_pointer_cast<MDEventWorkspace<MDLeanEvent<ND>, ND>>(ws))
    return {false, true};
  if(boost::dynamic_pointer_cast<MDEventWorkspace<MDEvent<ND>, ND>>(ws))
    return {true, true};
  return {false, false};
}

template <size_t ND_MAX>
std::pair<bool, size_t> getMDEventWSTypeND(API::IMDEventWorkspace_sptr ws) {
   auto res = isMDEventWSTypeND<ND_MAX>(ws);
   return res.second ? std::make_pair(res.first, ND_MAX) : getMDEventWSTypeND<ND_MAX - 1>(ws);
}

template <>
inline std::pair<bool, size_t> getMDEventWSTypeND<1>(API::IMDEventWorkspace_sptr ws) {
  auto res = isMDEventWSTypeND<1>(ws);
